...
client_worker.connect(async (err, conn) => {
  ...
  client.connect(async (err, conn) => {
    ...
    mapper._transform = function(obj, enc, cb) {
      ...
      const self = this
      client_worker.query(`
        update ss_sales set has_been_returned_from_these_salesids = (SELECT array_agg(distinct si.salesid) as has_been_returned_from_these_salesids 
        from public.ss_saleitems si where si.is_a_return_from_salesid = ${obj.salesid}) where salesid = ${obj.salesid}
      `)
      .then(res => {
        self.push(obj)
        cb();        
      ...
      .catch(err => {
        console.error('error in setting has_been_returned_from_these_salesids', err)
        throw err
      })
    }
    ...
    process.stdout.write(`Setting has_been_returned_from_these_salesids \n`)
